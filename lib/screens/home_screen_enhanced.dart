...
class _HomeScreenEnhancedState extends State<HomeScreenEnhanced>
  ...
  Future<void> _loadFoodItems() async {
    ...
    try {
      ...
      setState(() {
        _foodItems = items;
        _isLoading = false;
        _totalItems = stats['total'] ?? 0;
        _expiredItems = stats['expired'] ?? 0;
        _expiringSoonItems = 0; // 期限近いアイテム数を計算
  ...
  Future<List<FoodItem>> _searchFoodItems(String query) async {
    try {
      ...
      throw AppDatabaseException('Failed to search food items', originalError: e);
    }
  }
