...
void main() {
  ...
  group('Recipe 機能テスト', () {
    ...
    test('材料の内容を正しく取得できる', () {
      ...
      expect(recipe.ingredients.contains('テスト玉ねぎ'), true);
      expect(recipe.ingredients.contains('テスト人参'), true);
      expect(recipe.ingredients.contains('テストじゃがいも'), true);
      expect(recipe.ingredients.contains('テストカレールー'), true);
    });
    ...
    test('日本語レシピのタイトルを正しく取得できる', () {
      ...
      expect(japaneseRecipe.title, 'テストカレーライス');
      expect(japaneseRecipe.category, 'カレー');
      expect(japaneseRecipe.area, '日本');
    });
    ...
    test('手順の内容を正しく取得できる', () {
      ...
      expect(recipe.instructions, contains('テスト用カレーライスの作り方'));
      expect(recipe.instructions, contains('1. 材料を切る'));
      expect(recipe.instructions, contains('2. 炒める'));
      expect(recipe.instructions, contains('3. 調味する'));
    });
  });
}
