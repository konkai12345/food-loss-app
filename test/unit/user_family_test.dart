...
import 'package:food_loss_app/test/data/test_data.dart';
...
void main() {
  group('User 単体テスト', () {
    ...
    setUp(() {
      testUser = TestData.getTestUsers().first;
    });
  ...
  group('Family 単体テスト', () {
    ...
    setUp(() {
      testFamily = TestData.getTestFamilies().first;
    });
  ...
  group('FamilyMember 単体テスト', () {
    ...
    setUp(() {
      testFamilyMember = TestData.getTestFamilyMembers().first;
    });
  ...
  group('User-Family 関連テスト', () {
    ...
    test('ユーザーとファミリーの関連性が正しく機能する', () {
      final testUsers = TestData.getTestUsers();
      final testFamilies = TestData.getTestFamilies();
      final testMembers = TestData.getTestFamilyMembers();

      // ユーザー1はファミリー1に所属
      final user1 = testUsers.first;
      expect(user1.familyIds, ['test_family_1']);
      
      // ファミリー1にはユーザー1とユーザー2が所属
      final family1 = testFamilies.first;
      expect(family1.memberIds, ['test_user_1', 'test_user_2']);
      
      // メンバー1はユーザー1としてファミリー1に所属
      final member1 = testMembers.first;
      expect(member1.userId, 'test_user_1');
      expect(member1.familyId, 'test_family_1');
      expect(member1.role, 'admin');
    });
  });
