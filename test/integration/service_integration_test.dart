...
void main() {
  group('サービス間連携テスト', () {
    ...
    group('DatabaseServiceと外部API連携', () {
      ...
      test('ユーザーデータの永続化', () async {
        // テスト用ユーザーデータ
        final testUserId = 'test_user_integration';
        final testUserData = {
          'name': '統合テストユーザー',
        ...
        // ユーザーデータの保存
        await databaseService.saveUserData(testUserId, testUserData);

        // ユーザーデータの取得
        final savedUserData = await databaseService.getUserData(testUserId);
        expect(savedUserData, isNotNull);
        expect(savedUserData!['name'], '統合テストユーザー');
        expect(savedUserData['email'], 'integration@test.com');

        // クリーンアップ
        await databaseService.deleteUserData(testUserId);
      });
    });

    group('エラーハンドリングと例外処理', () {
      test('存在しない食品アイテムの取得', () async {
        final nonExistentItem = await databaseService.getFoodItem('non_existent_id');
    ...
    group('パフォーマンスと負荷テスト', () {
      test('大量データの挿入と取得', () async {
        ...
        stopwatch.stop();
        print('100件の挿入時間: ${stopwatch.elapsedMilliseconds}ms');
        ...
        final items = await databaseService.getFoodItems();
        
        stopwatch.stop();
        print('100件の取得時間: ${stopwatch.elapsedMilliseconds}ms');
    ...
    group('データ整合性テスト', () {
      test('トランザクション処理の確認', () async {
        ...
        try {
          ...
          // 両方のアイテムが存在することを確認
          final item1 = await databaseService.getFoodItem('transaction_test_1');
          final item2 = await databaseService.getFoodItem('transaction_test_2');
